...
class DQNAgent:
    def __init__(self, cfg: dict) -> None:
        ...
        self.qnet_target = QNet(self.state_size, self.action_size, self.device).to(self.device)
    ...
    def update(self, state: np.array, action: int, reward: float, next_state: np.array, done: bool) -> None:
        ...
        # qnet で現在の状態から行動を推定する
        qs = self.qnet(state)

        # action で選択した方の行動がq
        q = qs[np.arange(self.batch_size), action]

        # qnet_target でnextactionを推定する
        next_qs = self.qnet_target(next_state)
        next_q = next_qs.max(axis=1)[0].to('cpu').detach().numpy()

        # nextactionの価値関数を更新する
